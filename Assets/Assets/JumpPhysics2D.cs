...
public class JumpPhysics2D : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        else if (collidingLayerList != null && collidingLayerList.Count > 0)
        { // if we have layer list, ensure the layers actually exist in the game.
    ...
    bool RaycastCollideVertical(Vector2 direction)
    {
        ...
        if (playerLayerName.Length == 0)
        { // we haven't assigned player layer mask

            // cast rays for collision checking
            hitLeftSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.GetComponent<Renderer>().bounds.extents.y + directionDepth);
            hitRightSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.GetComponent<Renderer>().bounds.extents.y + directionDepth);

        }
        else
        { // player layer mask assigned
            ...
            // cast rays for collision checking
            hitLeftSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.GetComponent<Renderer>().bounds.extents.y + directionDepth, LayerMask.NameToLayer(playerLayerName));
            hitRightSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.GetComponent<Renderer>().bounds.extents.y + directionDepth, LayerMask.NameToLayer(playerLayerName));
        }
